-- Completion-B: extend test cleanup for opening balances + routed settlements
...
begin
  ...
  foreach v_id in array v_ids loop
    ...
    delete from public.record_comments where household_id = v_id;
    delete from public.expense_receipt_jobs where household_id = v_id;
    delete from public.expense_receipt_duplicates where household_id = v_id;
    delete from public.expense_receipt_line_items where household_id = v_id;
    delete from public.expense_receipt_extractions where household_id = v_id;
    delete from public.expense_receipts where household_id = v_id;
    delete from public.household_import_rows
      where batch_id in (
        select id from public.household_import_batches where household_id = v_id
      );
    delete from public.household_import_batches where household_id = v_id;
    delete from public.household_export_jobs where household_id = v_id;
    delete from public.household_setup_progress where household_id = v_id;

    delete from public.household_poll_votes where household_id = v_id;
    delete from public.household_poll_options where household_id = v_id;
    delete from public.household_polls where household_id = v_id;
    delete from public.household_utilities where household_id = v_id;
    ...
    -- Completion-B (before payments / obligations)
    delete from public.routed_settlement_reservations where household_id = v_id;
    delete from public.routed_settlement_payment_links where household_id = v_id;
    delete from public.routed_settlement_approvals where household_id = v_id;
    delete from public.routed_settlement_events where household_id = v_id;
    delete from public.routed_settlement_legs where household_id = v_id;
    update public.routed_settlement_proposals set payment_id = null where household_id = v_id;
    delete from public.routed_settlement_proposals where household_id = v_id;
    delete from public.opening_balance_import_links where household_id = v_id;
    delete from public.opening_balance_approvals where household_id = v_id;
    delete from public.opening_balance_events where household_id = v_id;
    update public.opening_balance_entries set obligation_id = null where household_id = v_id;
    delete from public.opening_balance_entries where household_id = v_id;
