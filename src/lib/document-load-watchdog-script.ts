 ...
 * or an explicit recovery surface. Runs even when layouts are blocked.
 */
export const DOCUMENT_LOAD_WATCHDOG_SCRIPT = `(function(){
  try {
    ...
    function ready() {
      return !!(
        document.querySelector('[data-testid="home-action-center"]') ||
        document.querySelector('[data-testid="route-load-guard-recovery"]') ||
        document.querySelector('[data-testid="household-error-boundary"]') ||
        document.querySelector('[data-testid="household-shell"]')
      );
    }
    function already() {
    ...
    function show() {
      if (!pathOk() || ready() || already()) return;
      ...
      el.innerHTML = "<p style=\\"margin:0 0 .35rem;font-weight:650\\">This page is taking too long to load.</p>"
        + "<p style=\\"margin:0 0 .75rem;font-weight:400;color:#44403c\\">You may already be signed in. Reload, choose another household, or sign out.</p>"
        ...
        + "<a href=\\"/app\\" style=\\"min-height:44px;padding:.5rem 1rem;border:1px solid #c4b8a5;border-radius:.375rem;background:#fff;color:#1c1917;font-weight:650;text-decoration:none;display:inline-flex;align-items:center\\">Choose household</a>"
